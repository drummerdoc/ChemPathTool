...
class Group:
    ...
    def awt(self,ckd):
        awt = 0
        for elt in self.mEltCnts.keys():
            awtElt = ckd.elementAtomicWt()[ckd.indexElt(elt)]
            awt = awt + abs(self.mEltCnts[elt])*awtElt
        return awt
    def size(self):
        return len(self.mEltCnts)

def getEdges(ckd,trElt):
    edges = []
    groups = {}
    for sp in ckd.speciesNames():
        eltCnts = {}
        for elt in ckd.elementNames():
            m = ckd.numberOfElementXinSpeciesY(elt,sp)
            if (m>0):
                eltCnts[elt] = m
        groups[sp] = Group(eltCnts)

    for r in range(ckd.numReactions()):
        Redges = []
        coeffs = ckd.specCoeffsInReactions(r)
        net = {}
        for (sp,co) in coeffs:
            if (net.has_key(sp)):
                net[sp] = net[sp] + co
        ...
        for sp in net.keys():
            if (net[sp] < 0):
                if (ckd.numberOfElementXinSpeciesY(trElt,sp)>0):
        ...
        if ((LR <= 1) | (LP <= 1)):
            ...
            if (LR == 2 & LP == 2):
                ...
                print 'Cannot decompose rxn: ',r,': ',ckd.reactionString(r)

#        line = '(%3d) %-25s: ' % (r+1,ckd.reactionString(r))
#        for edge in Redges:
