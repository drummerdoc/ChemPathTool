...
class GraphNode:
    ...
    def show(self):
        ...
        self.label = self.canvas.create_text(self.xc,self.yc,text=self.name)
...
class ChemPathTool:
    ...
    def buildWidgets(self,parent):
        ...
        self.oneOrTwo_menu = Pmw.OptionMenu(self.edgeP.interior(),
                                            ...
                                            items = list(self.coalesceMap.keys()),
        ...
        self.normalize_menu = Pmw.OptionMenu(self.edgeP.interior(),
                                             ...
                                             items = list(self.normalizeMap.keys()),
...
#                 ((s2==name) & (nodeMap.has_key(s1))) ):
            if ( ((s1==name) & (s2 in nodeMap)) |
                 ((s2==name) & (s1 in nodeMap)) ):
                tag = str(PairTag(s1,s2))
#                if (edgeMap.has_key(tag)==0):
                if (tag not in edgeMap):
                    tags.append(tag)
        return tags
...
#            if (edgeMap.has_key(str(tag))):
    ...
    def doReadEDat(self,ifile):
        ...
        for ln in range(2,len(l)):
            ...
            if p[0] not in nodes:
                print('Bad input line'+str(ln+1)+':'+str(p[0])+'not in node list...continuing')
            elif p[1] not in nodes:
                print('Bad input line'+str(ln+1)+':'+str(p[1])+'not in node list...continuing')
            else:
                vals = []
                vals.append(p[0])
                vals.append(p[1])
